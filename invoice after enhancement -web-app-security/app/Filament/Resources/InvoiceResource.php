            ...
            ->schema([
                \Filament\Forms\Components\Section::make('')
                    ...
                    ->schema([
                        ...
                        // FIX FOR FLAW 3: Input Sanitization & Length Restrictions (Stored XSS Defense)
                        Forms\Components\Textarea::make('notes')
                            ->maxLength(5000) 
                            ->dehydrateStateUsing(function ($state) {
                                // Strips out raw HTML/JavaScript tags completely before saving to the database
                        ...
                        TableRepeater::make('item_invoice')
                            ...
                            ->schema([
                                ...
                                // FIX FOR FLAW 1: Range and Numeric Boundary Check (Negative Numbers Defense)
                                \Filament\Forms\Components\TextInput::make('pivot.quantity')
                                    ->integer()
                                    ->minValue(1) // Secure Fix: Blocks 0 and all negative integers on the server level
                                    ->required()
                                    ->afterStateUpdated(function ($set, $get, $state) {
                                        self::calculate_total($set, $get);
                                    }),
